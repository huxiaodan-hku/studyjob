...
const ApplicationPanel = props => {
  const unfinishTaskRef = React.useRef(null);
  const allTaskRef = React.useRef(null);
  ...
  const refreshTask = () => {
	...
	const goBottom = () => {
		unfinishTaskRef.current.scrollTop = unfinishTaskRef.current.scrollHeight
		allTaskRef.current.scrollTop = allTaskRef.current.scrollHeight
	}
  const allTasks = useLoadTasks(false, groupId, shouldRefresh, setShouldRefresh);
  const notFinishedTasks = useLoadTasks(true, groupId, shouldRefresh, setShouldRefresh);
  return (<div className={classes.root}>
    ...
    <div className={classes.content}>
      <div className={classes.demo1}>
        <AntTabs value={value} onChange={handleChange} indicatorColor="primary" textColor="primary" aria-label="full width tabs example">
          ...
          <IconButton aria-label="refresh" color="primary">
            <RefreshIcon onClick={refreshTask}/>
          </IconButton>
					<IconButton aria-label="goBottom" color="primary">
            <ArrowDownwardIcon onClick={goBottom}/>
          </IconButton>
      ...
      <SwipeableViews axis={theme.direction === 'rtl'
        ...
        <div className={classes.tabPanel} ref={unfinishTaskRef}>
          <TabPanel value={value} index={0} dir={theme.direction}>
            {
              notFinishedTasks.map(task => {
                return (<ThumbTask postTime={task.postDate} imgUrl={task.posterUrl} taskDec={task.taskDec} posterName={task.posterName} taskDetail={task.taskDetail} taskName={task.taskName} messageId={task.messageId}/>)
              })
            }
          </TabPanel>
        </div>
        <div className={classes.tabPanel} ref={allTaskRef}>
